...
public class GainProvinceAndCityAndAreaController {
	...
	/**
	 * 该方法用于selectArea.jsp页面的城市选择
	 * 当选择完毕通过ajax访问到这个control
	 * @param session
	 * 用于保存地址
	 * @param province
	 * 地址的省
	...
	@ResponseBody
	public String ajaxKeepPlace(HttpSession session,String province,String city){
	...
	/**
	 * 该方法用于点击selectArea.jsp页面的a标签对应的地址
	 * 点击通过ajax访问这个control
	 * @param session
	 * 用于保存地址
	 * @param province
	 ...
	 * 地点对应的市
	 * @return
	 * 返回index,直接调转到index界面
	 */
	@RequestMapping("/keepPlace/and/jumpIndex")
	public String aKeepPlace(HttpSession session,String province,String city){
		session.setAttribute("placeSession", city);
		session.setAttribute("provinceSession", province);
    	return "index";
    }
